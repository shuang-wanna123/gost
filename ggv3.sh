...
install_gost() {
    ...
    cd /tmp || return 1
    rm -f gost.tar.gz gost 2>/dev/null

    if command -v wget &>/dev/null; then
        wget -q --show-progress -O gost.tar.gz "$url" || { msg_error "下载失败"; return 1; }
    elif command -v curl &>/dev/null; then
        curl -L -o gost.tar.gz "$url" || { msg_error "下载失败"; return 1; }
    else
        msg_error "请先安装 wget 或 curl"
        return 1
    fi

    tar -xzf gost.tar.gz gost || { msg_error "解压失败"; return 1; }
    chmod +x gost
    mv -f gost "$GOST_BIN"
    rm -f gost.tar.gz
...
add_landing() {
    ...
    echo "  3. aes-128-gcm"
    echo ""
    read -rp "请选择加密方式 [回车默认: 1]: " method_choice
